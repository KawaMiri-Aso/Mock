...
void CStoneTrap::Step()
{
	...
	if (m_eState == STONE_TRAP_STATE_BEFORE)
	{
		...
		{
			//プレイヤーと当たり判定とる
			if (CCollision::IsHitSphere(m_vPos, STONE_RAD, player->GetPos(), PLAYER_RAD))
			{
				//プレイヤーが押す方向に押し出す
...
void CStoneTrap::CheckCollision()
{
	...
	//			float bottom = m_vPos.y - STONE_RAD;
	//			// 床から足元までの距離を計算
	//			float dist = bottom - floor_height;
	...
	//			//プレイヤーの脳天の座標を計算
	//			float top = m_vPos.y + STONE_RAD;
	//			//天井から脳天までの距離を計算
	//			float dist = top - ceil_height;
	...
	//					//衝突点までの距離を算出
	//					float front = norm.z < 0.0f ? m_vPos.z + STONE_RAD : m_vPos.z - STONE_RAD;
	//					float dist = front - wall_height;
	//					//壁から離れすぎていないかチェック
	...
	//					//衝突点までの距離を算出
	//					float front = norm.x < 0.0f ? m_vPos.x + STONE_RAD : m_vPos.x - STONE_RAD;
	//					float dist = front - wall_height;
	//					//壁から離れすぎていないかチェック
	//					if (MyMath::Abs(dist) <= STONE_RAD)
	//					{
...
//現在位置に座標加算
void CStoneTrap::ReflectCollision(VECTOR vAddVec)
{
	// オールゼロなら何もしない
	if (vAddVec.x == 0.0f && vAddVec.y == 0.0f && vAddVec.z == 0.0f) return;

	VECTOR	vPos = MV1GetPosition(m_nHandle);
